...
"진실의 단일 원천"이라는 뜻입니다. **하나의 데이터는 한 곳에서만 관리**하자는 원칙입니다.

이번 작업에서는 `index.css`를 SSOT로 삼았습니다:
...
만약 폰트 크기가 `tokens.ts`, `index.css`, `tailwind.config.js` 세 곳에 각각 정의되어 있다면? 하나를 수정했을 때 나머지를 깜빡하면 버그가 됩니다.

한 곳에서만 관리하면 이런 실수가 원천 차단됩니다.

#### 어떻게 탄생했어?

데이터베이스 정규화 이론에서 시작된 개념입니다. "데이터 중복을 피하라"는 원칙이 소프트웨어 전반으로 확장되었습니다.

Redux 같은 상태 관리 라이브러리도 이 원칙을 따릅니다 - 앱 상태를 한 곳(store)에서만 관리하죠.
...
- **의존성 증가**: 그 한 곳이 망가지면 전체가 영향받음
- **설계 비용**: 어디를 SSOT로 삼을지 결정 필요
...
A: 아니요. **런타임에 바뀔 가능성이 있는 값**만 CSS 변수로 만들면 됩니다. 색상, 폰트 크기(접근성), 트랜지션(모션 감소 설정) 등이 해당됩니다. 스페이싱은 보통 안 바뀌지만, 이 프로젝트에서는 **문서화(SSOT)** 목적으로 연동했습니다.

### Q: SSOT를 위해서라면 tokens.ts를 SSOT로 삼아도 되지 않아?

A: 가능합니다. 하지만 `tokens.ts`를 SSOT로 삼으면 빌드 타임에 값이 고정됩니다. `index.css`를 SSOT로 삼으면 런타임 변경이 가능하고, 브라우저 개발자 도구에서 바로 값을 확인할 수 있어 디버깅도 쉽습니다.

### Q: tailwind.config.js에서 직접 값을 정의하면 안 돼?

A: 가능하지만, 그러면 값이 두 군데(tailwind.config.js, index.css)에 분산됩니다. 다크 모드를 위해 index.css에도 색상을 정의해야 하니까요. 이러면 SSOT가 깨집니다.
...
> 디자인 토큰은 CSS 변수로 정의하고, Tailwind가 참조하게 하면 **한 곳(SSOT)에서 모든 스타일을 관리**할 수 있다.
