...
- **역할**: 유스케이스의 **지휘자(Orchestrator)**. 복잡한 비즈니스 로직을 직접 구현하는 대신, 여러 **똑똑한 도메인 객체(Rich Domain Objects)**들을 조율하여 유스케이스를 완성합니다. 서비스는 도메인 객체에게 작업을 **'시키고(Tell)'**, 그 결과를 받아 흐름을 제어하는 **얇은 계층**이어야 합니다.
- **지침**:
  - 각 서비스는 단일 유스케이스 인터페이스를 구현합니다.
  - **모든 핵심 비즈니스 로직은 도메인 객체에 위임합니다.**
  - 서비스는 트랜잭션 관리, 외부 시스템(Port) 호출, 도메인 객체 간의 흐름 제어만 담당합니다.
  - **서비스의 public 메서드는 15~20줄을 넘지 않도록 간결하게 유지합니다.**
  - 메서드가 길어지거나 여러 책임을 수행한다면, 로직을 별도의 도메인 객체나 다른 도메인 서비스로 분리하는 리팩토링을 수행합니다.
