...
public class TestcontainersModule {
    ...
    /**
     * 공통 PostgreSQL 컨테이너 설정
     * - PostgreSQL 15 Alpine 버전 사용 (경량화)
     * - 모든 테스트에서 동일한 PostgreSQL 환경 사용
     */
    ...
    /**
     * MySQL 컨테이너 시작 여부를 추적하는 플래그
     * - mysql.isRunning() 호출 없이 빠르게 상태 확인
     * - 한 번 시작되면 JVM 종료까지 계속 실행 상태 유지
     */
    private static volatile boolean isMySQLContainerStarted = false;
    
    /**
     * PostgreSQL 컨테이너 시작 여부를 추적하는 플래그
     * - postgres.isRunning() 호출 없이 빠르게 상태 확인
     * - 한 번 시작되면 JVM 종료까지 계속 실행 상태 유지
     */
    private static volatile boolean isPostgreSQLContainerStarted = false;
    ...
    /**
     * PostgreSQL 컨테이너를 시작하고 Spring 데이터소스 시스템 프로퍼티를 설정합니다.
     * 
     * 최적화:
     * - 자바 플래그로 먼저 체크 (postgres.isRunning() 호출 방지)
     * - 한 번 시작되면 재호출 시 즉시 반환
     * 
     * 이 메서드는 클래스 로딩 시점에 한 번만 실행되어야 하므로,
     * 각 베이스 클래스의 static 블록에서 호출해야 합니다.
     */
    public static void startPostgreSQLContainer() {
        // 플래그 먼저 체크 - 컨테이너 호출 없이 빠르게 반환
        if (isPostgreSQLContainerStarted) {
            return;
        }
        ...
        synchronized (TestcontainersModule.class) {
            ...
            // 플래그 설정 - 이후 호출들은 즉시 반환
            isPostgreSQLContainerStarted = true;
            
            System.out.println("🐘 PostgreSQL Testcontainer started: " + postgres.getJdbcUrl());
        }
    }
