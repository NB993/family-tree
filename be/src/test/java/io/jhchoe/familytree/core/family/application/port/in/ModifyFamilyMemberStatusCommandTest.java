...
/**
 * ModifyFamilyMemberStatusCommand 클래스의 단위 테스트입니다.
 */
@DisplayName("[Unit Test] ModifyFamilyMemberStatusCommandTest")
class ModifyFamilyMemberStatusCommandTest {
    ...
    @Test
    @DisplayName("reason이 null이어도 명령 객체를 생성할 수 있습니다")
    void create_command_with_null_reason() {
        // given
        Long familyId = 1L;
        Long memberId = 2L;
        Long currentUserId = 3L;
    ...
    void throw_exception_when_family_id_is_null() {
        ...
        // when & then
        assertThatThrownBy(() -> new ModifyFamilyMemberStatusCommand(
            familyId, memberId, currentUserId, newStatus, reason
        ))
    ...
    @Test
    @DisplayName("newStatus가 null인 경우 예외가 발생합니다")
    void throw_exception_when_new_status_is_null() {
        // given
        Long familyId = 1L;
        Long memberId = 2L;
        Long currentUserId = 3L;
